...
export default function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [alunosRes, disciplinasRes, livrosRes, pedidosRes] = await Promise.all([
                ...
                <div className="space-y-4">
                  {recentPedidos.map((pedido) => (
                    <div key={pedido.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div>
                        <p className="font-medium">Pedido #{pedido.id.substring(0, 8)}</p>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-green-600">{stats.pedidosPagos}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Aguardando Pagamento</span>
                  <span className="font-semibold text-yellow-600">{stats.pedidosPendentes}</span>
                </div>
                <div className="flex justify-between items-center">
